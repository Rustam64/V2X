...
#define LEFT A0               // Left IR sensor connected to analog pin A3 of Arduino Uno:
...
#define MAX_DISTANCE 30      // Maximum ping distance:
...
unsigned int Right_Value; //Variable to store Right IR sensor value:
unsigned int Left_Value;  //Variable to store Left IR sensor value:
...
if((distance > 0) && (distance < 10)){            //check wheather the ultrasonic sensor's value stays between 1 to 15.
...
}else if((distance > 20)||(distance==0)) {                          //If the condition is 'true' then the statement below will execute:
